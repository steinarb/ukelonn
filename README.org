...
Dette krever at man har en lokal PostgreSQL-installasjon med følgende innstillinger:
 ...
 2. Sett opp autentiseringsmetode trust på windows, ident auth på debian (krever en ident-demon)
...
Framgangsmåte:
 ...
 2. [[http://karaf.apache.org/download.html][Last ned Karaf 4.1.1]], pakk ut den nedlastede distribusjonen og start karaf fra kommandlinja
    #+BEGIN_EXAMPLE
      ...
      chown -R karaf.karaf apache-karaf-4.1.1
      cd apache-karaf-4.1.1
      /bin/sudo karaf bin/karaf
    #+END_EXAMPLE
    (kommandoeksempler fra debian, kommandoer kjørt som root)
