...
Prosedyre (alle kommandolinje-eksempler gjort i bash, logget inn som root):
 ...
 5. Sjekk ut og bygg webappen som bruker karaf:
    #+BEGIN_EXAMPLE
      ...
      /usr/bin/sudo -u karaf /bin/bash -c 'mkdir -p ~/git; cd ~/git; git clone https://github.com/steinarb/ukelonn.git; cd ukelonn; git checkout using-vaadin; mvn install -DskipTests=true -Dmaven.javadoc.skip=true'
    #+END_EXAMPLE
