...
Framgangsmåten er som følger:
 1. Følge Apache Karaf quick start guide (NB! last ned og bruk karaf 4.1.7, ikke karaf 4.2.x) https://karaf.apache.org/manual/latest/quick-start.html
...
Dette trengs for å bygge og kjøre programvaren
 ...
 3. I kommandolinje-vinduet, bygg applikasjonen med følgende kommandoer:
    #+BEGIN_EXAMPLE
      ...
      mvn clean install
    #+END_EXAMPLE
 4. Følge Apache Karaf quick start guide (NB! last ned og bruk karaf 4.1.7, ikke karaf 4.2.x) https://karaf.apache.org/manual/latest/quick-start.html
...
Framgangsmåte:
 1. Klon og bygg webapplikasjonen (kommandoeksemplene er fra bash på GNU/linux):
    #+BEGIN_EXAMPLE
      ...
      mvn clean install
    #+END_EXAMPLE
 2. [[http://karaf.apache.org/download.html][Last ned Karaf 4.1.7]], pakk ut den nedlastede distribusjonen og start karaf fra kommandlinja (karaf vil da kjøre som din egen bruker som stemmer med PostgreSQL-oppsettet i starten av dette avsnittet):
    #+BEGIN_EXAMPLE
      cd /tmp
      wget http://www.apache.org/dyn/closer.lua/karaf/4.1.7/apache-karaf-4.1.7.tar.gz
      cd ~
      tar xvfz /tmp/apache-karaf-4.1.7.tar.gz
      cd apache-karaf-4.1.7
      bin/karaf
    #+END_EXAMPLE
