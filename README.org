...
Framgangsmåten er som følger:
 ...
 2. Fra kommandolinja til karaf gjør noen konfigureringer (konfigurasjonene blir perstert i etc-katalogen til karaf og trengs ikke å gjøres om igjen):
    1. Legg til et maven-repo som inneholder ukelonn bygd fra travis-ci
       #+BEGIN_EXAMPLE
         ...
         config:update
       #+END_EXAMPLE
    2. Endre JDBC-konfig for authservice til å bruke samme derby-database som ukelonn:
       #+begin_example
         ...
         config:property-set org.apache.karaf.features.configKey "org.ops4j.datasource-authservice-test"
         config:property-set url "jdbc:derby:memory:ukelonn;create=true"
         config:update
       #+end_example
    3. Stopp karaf og start karaf igjen for at endringen til derby-databasen skal fanges opp
