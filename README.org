...
To set the default to English, do the following commands in the karaf console:
#+begin_example
  config:edit no.priv.bang.ukelonn.backend.UkelonnServiceProvider
  config:property-set defaultlocale en_UK
  config:update
#+end_example

* Status
/NB!/ Byggefeilen skyldes et problem med travis-ci som jeg ikke skjønner
/Note/ The build failure is an issue with travis-ci that I've been unable to figure out
