...
public class CommonDatabaseMethodsTest {
    ...
    public void testGetAccountInfoFromDatabase() {
        ...
        List<Transaction> jobs = getJobsFromAccount(getUkelonnServletProvider(), account, getClass());
        assertEquals(10, jobs.size());
        List<Transaction> payments = getPaymentsFromAccount(getUkelonnServletProvider(), account, getClass());
    ...
    public void testAddPaymentTypeToDatabase() {
        // Verify precondition
        List<TransactionType> jobTypesBefore = getPaymentTypesFromTransactionTypes(getTransactionTypesFromUkelonnDatabase(getUkelonnServletProvider(), getClass()).values());
        assertEquals(2, jobTypesBefore.size());

        addPaymentTypeToDatabase(getUkelonnServletProvider(), getClass(), "Sjekk", null);

        // Verify that a job has been added
        List<TransactionType> jobTypesAfter = getPaymentTypesFromTransactionTypes(getTransactionTypesFromUkelonnDatabase(getUkelonnServletProvider(), getClass()).values());
    ...
    public void testAddUserToDatabase() {
        // Verify precondition
        List<User> usersBefore = getUsers(getUkelonnServletProvider(), getClass());
        assertEquals(5, usersBefore.size());

        addUserToDatabase(getUkelonnServletProvider(), getClass(), "un", "zecret", "un@gmail.com", "User", "Name");

        // Verify that a user has been added
        List<User> usersAfter = getUsers(getUkelonnServletProvider(), getClass());
    ...
    public void testChangePasswordForUser() {
        ...
        changePasswordForUser(getUkelonnServletProvider(), username, newPassword, getClass());
    ...
    public void testUpdateTransactionTypeInDatabase() {
        ...
        Map<Integer, TransactionType> transactionTypes = getTransactionTypesFromUkelonnDatabase(getUkelonnServletProvider(), getClass());
        ...
        updateTransactionTypeInDatabase(getUkelonnServletProvider(), getClass(), transactionTypeBeforeModification);

        // Verify the changed state of the transaction type in the database
        transactionTypes = getTransactionTypesFromUkelonnDatabase(getUkelonnServletProvider(), getClass());
    ...
    public void testDeleteTransactions() {
        ...
        List<Transaction> initialJobsForJod = getJobsFromAccount(getUkelonnServletProvider(), account, getClass());
        assertEquals(2, initialJobsForJod.size());

        // Add two jobs that are to be deleted later
        registerNewJobInDatabase(getUkelonnServletProvider(), getClass(), account, 1, 45);
        registerNewJobInDatabase(getUkelonnServletProvider(), getClass(), account, 2, 45);
        ...
        deleteTransactions(getUkelonnServletProvider(), getClass(), jobsToDelete);

        // Verify that the jobs has been deleted
        List<Transaction> jobsAfterDelete = getJobsFromAccount(getUkelonnServletProvider(), account, getClass());
    ...
    public void testAddNewPaymentToAccount() {
        ...
        Account account = getAccountInfoFromDatabase(getUkelonnServletProvider(), getClass(), "jod");
        List<Transaction> initialPaymentsForJod = getPaymentsFromAccount(getUkelonnServletProvider(), account, getClass());
        assertEquals(1, initialPaymentsForJod.size());

        // Register a payment
        Map<Integer, TransactionType> transactionTypes = getTransactionTypesFromUkelonnDatabase(getUkelonnServletProvider(), getClass());
        addNewPaymentToAccount(getUkelonnServletProvider(), getClass(), account, transactionTypes.get(4), account.getBalance());
